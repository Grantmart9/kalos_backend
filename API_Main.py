...
@app.route('/put_order_auth', methods=['GET'])
def put_order_auth():
    conn = None
    incoming_data = request.json
    token = request.headers['token']
    ...
    try:
        ...
        if jwt_token is not None:
            ...
            # 2) If exists, copy OTP out of DB. If not exist, create new OTP and store New OTP in DB
            # 3) return OTP to client.
            print("sending OTP to:" + str(otp))
        if jwt_token is None:
            print("User not authorise")
    except (Exception, psycopg2.DatabaseError) as error:
        print("Error while quering DB")
